...
class _paymentSuccessState extends State<PaymentSuccessScreen>{
  ...
  void initState() {
    ...
    }else if(widget.paymentType==5){
      paymentTitle="Thank you for registering for the class! Your registration has been confirmed. We're delighted to have you with us and look forward to seeing you soon.";
    }else{
  ...
  YogaClassSuccess() async {
    ...
    try{
      var response = await helper.getAPIForNormalResponse(
          'yoga_class_management/paymentSuccess/${widget.orderId}', context);
      print("RAW RESPONSE:");
      print(response.body);
      if (response.body.trim().startsWith('<')) {
        isSuccess = true;
        OrderStatus = "Thank you for registering for the class! Your registration has been confirmed. We're delighted to have you with us and look forward to seeing you soon.";
        paymentTitle = OrderStatus;
      } else {
        final responseJSON = jsonDecode(response.body);
        if (responseJSON['statusCode'] != null && responseJSON['statusCode'] == 400) {
          ...
          isSuccess = true;
          OrderStatus = responseJSON['message'] ?? "Thank you for registering for the class! Your registration has been confirmed. We're delighted to have you with us and look forward to seeing you soon.";
          paymentTitle = OrderStatus;
        }
      }
